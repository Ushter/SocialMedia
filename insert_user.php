...
if (isset($_POST['sign_up'])) {
    ...
    $run_email = mysqli_query($check_email, $co);
    ...
    $insert = "insert into users (f_name,l_name,user_name,describe_user,relationship,user_pass,user_email,user_country,user_gender,user_birthday,user_image,user_cover,user_reg_date,status,posts,recover_account)
		values('$first_name','$last_name','$username','','...','$pass','$email','$country','$gender','$birthday','$profile_pic','',NOW(),'$status','$posts','Recover account.')";

    $query = mysqli_query($con, $insert);

    if ($query) {
        echo "<script>alert(' $first_name, you are signed up.')</script>";
